        ...
        setTimeout(function () {
            ...
            if (connection.request.url.endsWith('/api/authenticate') && connection.request.method === http_1.RequestMethod.Post) {
                ...
                var filteredUsers = users.filter(function (user) {
                    return user.login === params_1.login && user.password === params_1.password;
                });
            ...
            if (connection.request.url.endsWith('/api/users') && connection.request.method === http_1.RequestMethod.Post) {
                ...
                // validation
                var duplicateUser = users.filter(function (user) { return user.login === newUser_1.login; }).length;
                if (duplicateUser) {
                    return connection.mockError(new Error('Username "' + newUser_1.login + '" is already taken'));
                }
                // save new user
